...
class Elementor_Widget_miga_posts extends \Elementor\Widget_Base
{
    ...
    protected function render()
    {
        ...
        $textalign = "miga_pos_left";
        if (in_array(esc_attr($settings["show_next_alignment"]), ['miga_pos_left','miga_pos_center','miga_pos_right'])) {
            $textalign = esc_attr($settings["show_next_alignment"]);
        }
        $btnalign = "left";
        if (in_array(esc_attr($settings["btnmore_alignment"]), ['left','center','right'])) {
            $btnalign = esc_attr($settings["btnmore_alignment"]);
        }
        ...
        foreach ($myposts as $post):
            ...
            if ($settings["show_next"] === "yes") {
                ...
                if ($settings["show_next_margin"] != "0") {
                    if ($textalign == "miga_pos_left") {
                        $template .=
                            "left:" . (int)$settings["show_next_margin"] . "px;";
                        $templateEmpty .=
                            "left:" . (int)$settings["show_next_margin"] . "px;";
                    } elseif (
                        ...
                        $template .=
                            "right:" . (int)$settings["show_next_margin"] . "px;";
                        $templateEmpty .=
                            "right:" . (int)$settings["show_next_margin"] . "px;";
                    }
                }
                ...
                if (isset($settings["show_next_margin_bottom"])) {
                    $template .=
                        "bottom:" .
                        (int)$settings["show_next_margin_bottom"] .
                        "px;";
                    $templateEmpty .=
                        "bottom:" .
                        (int)$settings["show_next_margin_bottom"] .
                ...
                $template .= '">' . esc_html($settings["show_next_text"]) . "</div>";
                $templateEmpty .= '">' . esc_html($settings["show_next_text"]) . "</div>";
            }
