...
class Elementor_Widget_miga_post_title extends \Elementor\Widget_Base
{
    ...
    protected function render()
    {
        ...
        if (in_array(esc_attr($settings["miga_custom_posts_title_tag"]), ['h1','h2','h3','h4','p','div'])) {
            $htmltag = esc_attr($settings["miga_custom_posts_title_tag"]);
        }
        if (in_array(esc_attr($settings["miga_custom_posts_title_align"]), ['left','center','right'])) {
            $textalign = esc_attr($settings["miga_custom_posts_title_align"]);
        }
