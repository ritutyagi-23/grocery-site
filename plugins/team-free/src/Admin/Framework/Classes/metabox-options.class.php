...
if ( ! class_exists( 'TEAMFW_Metabox' ) ) {
	...
	class TEAMFW_Metabox extends TEAMFW_Abstract {
		...
		private function transient_purge() {
			...
			$sql        = $wpdb->prepare( " SELECT option_name FROM $options WHERE option_name LIKE '%s' ", $t );// @codingStandardsIgnoreLine.
			$transients = $wpdb->get_col( $sql );// @codingStandardsIgnoreLine.
			// For each transient...
